...
NuclearPowerPlant::NuclearPowerPlant()
{
	...
	nuclear_waste = 3.4E-9; //tons of high level nuclear waste produced per kWh (it is stored to be reused when radiation decays)
	natural_uranium = 3.4E-8; //tons of natural uranium necessary per kWh
	fissile_material = 1.4E-7; //kg of fissile material needed per kWh
...
void NuclearPowerPlant::simulate_step(double days) 
{
	...
	fissile_material += 1.4E-7 * energy_per_day * days;
	natural_uranium += 3.4E-8 * energy_per_day * days;
	CO2_output += 0.012 * energy_per_day * days;
	nuclear_waste += 3.4E-9 * energy_per_day * days;
	radiation += 1.4E-12 * energy_per_day * days;
	environmental_cost = 0.06 * energy_per_day * days;
	maintenance += 166 * energy_per_day * days;
	if (total_days >= 3650) {
