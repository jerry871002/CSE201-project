...
void Player::_process(float delta) 
{
	...
	worldEnv->get_environment()->set_dof_blur_far_amount(0.15 * pow((1 - (this->get_global_transform().get_origin().y - MinHeight) / (MaxHeight - MinHeight)), 0.5) );
	
	this->get_child(0)->set("far", pow(get_global_transform().get_origin().y, 0.5)*30);
