...
void City::generate_initial_city_graphics()
{
	...
	std::cout << "DEBUG: generate_initial_city_graphics"  << std::endl;
	...
	for (int x = 0; x < citysize; x++) {
		for (int z = 0; z < citysize; z++) {
			...
			std::cout << "DEBUG: Big building maybe" << bigbuildingmaybe << std::endl;
...
void City::change_pie_chart(int totalSatisfaction, NodePath name)
{
	...
	node->set_tint_progress(red);
	if (totalSatisfaction < 70) {
		node->set_tint_progress(orange);
	}
